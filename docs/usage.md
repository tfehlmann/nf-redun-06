...
```console
nextflow run tfehlmann/nf-redun-06 --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull tfehlmann/nf-redun-06
```
...
First, go to the [tfehlmann/nf-redun-06 releases page](https://github.com/tfehlmann/nf-redun-06/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
