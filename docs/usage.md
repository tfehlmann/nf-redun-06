# nf-core/snrnaseq: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/snrnaseq/usage](https://nf-co.re/snrnaseq/usage)
...
```console
nextflow run nf-core/snrnaseq --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/snrnaseq
```
...
First, go to the [nf-core/snrnaseq releases page](https://github.com/nf-core/snrnaseq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
